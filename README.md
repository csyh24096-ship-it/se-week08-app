...
## SupabaseのSQLeditor上でのコード
### ログイン認証のためのSQLコード
-- 1. peopleテーブルに、誰が登録したかを記録する user_id カラムを追加
ALTER TABLE people ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) DEFAULT auth.uid();

-- 2. 前回無効化したRLS（セキュリティガード）を、安全のために「有効化」に戻す
ALTER TABLE people ENABLE ROW LEVEL SECURITY;

-- 3. ポリシー作成：ログインしている本人のデータだけを「閲覧（SELECT）」できるようにする
CREATE POLICY "ユーザーは自分の登録した人物だけを閲覧できる" 
ON people FOR SELECT 
TO authenticated 
USING (auth.uid() = user_id);

-- 4. ポリシー作成：ログインしている本人のデータとして「追加（INSERT）」できるようにする
CREATE POLICY "ユーザーは自分のデータとして人物を追加できる" 
...
CREATE TABLE public.memos (
  ...
  person_id UUID REFERENCES public.people(id) ON DELETE CASCADE NOT NULL, -- どの人物のメモか（peopleテーブルと連携）
  content TEXT NOT NULL,                                                  -- メモの内容
  user_id UUID REFERENCES auth.users(id) NOT NULL,                        -- 誰が書いたメモか（セキュリティ用）
...
-- 3. RLSポリシー（ログイン中の自分のメモだけを見書きできるルール）
...
[〇] 新しい人物（例：「田中太郎」）を入力し、一覧に追加されること（他人からは見えないこと）。

[〇] 「田中太郎」を選択し、メモ（例：「コーヒーはブラック派」）を入力・送信できること。

[〇] 画面をリロードしても、「田中太郎」の詳細画面に先ほどのメモが正しい日時で表示されていること。

[〇] 人物を削除した際、その人物に紐づいていたメモデータもデータベース上から削除されること。
